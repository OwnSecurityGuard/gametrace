...
// checkRulesJSONFor 解析项目当前生效（已启用且条件非空）的检查规则，序列化为
// 抓包启动时下发的 check_rules_json 列表。用于把项目配置注入流水线求值。
//
// 任何失败（项目缺失、调用方无读权限、序列化错误）都返回 nil —— 抓包启动绝不能
// 因为规则解析失败而失败，降级为「本会话不做规则检查」即可。
func (m *mcpCapture) checkRulesJSONFor(ctx context.Context, projectID string) []string {
	if projectID == "" || m.projects == nil {
		return nil
	}
	p, err := m.projectForAction(ctx, projectID, authz.ActionProjectRead)
	if err != nil {
		// 不可见/不存在统一静默：调用方可能本就没有该项目的读权限。
		return nil
	}
	active := checkrule.Active(p.Rules)
	if len(active) == 0 {
		return nil
	}
	return checkrule.EncodeList(active)
}
