...
// projectRule 是项目关联的一条检查规则。
//
// 历史上只是 {id, name} 的纯展示占位；现升级为结构化的 checkrule.CheckRule
// （命中条件 + 通知文案 + 冷却/上下文参数），由流水线在解码事件上求值。
// 用类型别名而非新类型，使既有的 rules JSON 列读写、scanProject、
// parseAssociatedEntries[projectRule] 等全部无改动继续工作。
type projectRule = checkrule.CheckRule
