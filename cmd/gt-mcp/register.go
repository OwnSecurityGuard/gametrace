// register.go — 自助注册：直接注册获取独立身份（2026-09-05）。
//
// 设计：
...
//   - 身份落 users 表（与既有注册用户同表、同一条 DBResolver 解析链），即时生效。
//   - 保留名拒绝：env bootstrap 的 owner（同名会让 projects/sessions 的 owner
//     字段把两个身份混同，权限边界击穿）、匿名 owner "local"、既有 users 表用户。
//   - token 仅创建时返回一次，与注册凭证同待遇。不做限速：部署假定内网/受信
...
func (m *mcpCapture) handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	// createdBy 空 = 自主注册（区别于由他人添加：created_by=添加人）。
