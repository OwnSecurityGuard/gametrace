...
// TestApplyDefaultSemanticsByDirection 覆盖宿主默认 request/response 语义：
// 不声明任何规则时按方向打标；显式 annotate / 插件自报 semantic 优先；
// 方向判不出来时保持无标签（不猜）。
func TestApplyDefaultSemanticsByDirection(t *testing.T) {
	e := newSemanticEngine(slog.New(slog.NewTextHandler(io.Discard, nil)), nil)
	mk := func(direction string, meta map[string]event.Value) *event.Event {
		ev := event.NewEvent("sess", event.EventType("game.message"), "",
			event.ValueObject(map[string]event.Value{"seq": event.ValueInt(1)}),
			event.EventContext{Direction: direction})
		if meta != nil {
	...
	req := mk("client_to_server", nil)
	e.enrichSemantics(req)
	assertLabels(t, semanticsOf(t, req), "request")

	resp := mk("server_to_client", nil)
	e.enrichSemantics(resp)
	assertLabels(t, semanticsOf(t, resp), "response")

	unknown := mk("", nil)
	e.enrichSemantics(unknown)
	if got := semanticsOf(t, unknown); got != nil {
		t.Errorf("方向未知不该有默认语义，got %v", got)
	}

	// 插件在 Meta 里自报 semantic：视为显式声明，默认值不覆盖。
	self := mk("server_to_client", map[string]event.Value{
		"semantic": event.ValueArray([]event.Value{event.ValueString("notification")}),
	})
	e.enrichSemantics(self)
	assertLabels(t, semanticsOf(t, self), "notification")
}

// TestDefaultSemanticsYieldsToAnnotate 覆盖 annotate 与默认的优先级：
// annotate 出角色的事件不被默认值改写；annotate 只打 error（状态属性、非角色）时
// 默认角色照常追加，两者并存；未命中的（哪怕规则集非空）仍享受默认。
func TestDefaultSemanticsYieldsToAnnotate(t *testing.T) {
	const manifest = `api_version: gt.decoder/v2
name: demo
protocol: demo
type: decoder
...
`
	...
	mk := func(payload map[string]event.Value, direction string) *event.Event {
		return event.NewEvent("sess", event.EventType("game.message"), "",
			event.ValueObject(payload), event.EventContext{Direction: direction})
	}

	push := mk(map[string]event.Value{"seq": event.ValueInt(0)}, "server_to_client")
	e.enrichSemantics(push)
	assertLabels(t, semanticsOf(t, push), "notification")

	resp := mk(map[string]event.Value{"seq": event.ValueInt(1)}, "server_to_client")
	e.enrichSemantics(resp)
	assertLabels(t, semanticsOf(t, resp), "response")

	// error 不是角色：默认 response 追加共存，与 lp「response+error 并存」的既有形态一致。
	bad := mk(map[string]event.Value{"seq": event.ValueInt(2), "error_code": event.ValueInt(7)}, "server_to_client")
	e.enrichSemantics(bad)
	assertLabels(t, semanticsOf(t, bad), "error", "response")
}
