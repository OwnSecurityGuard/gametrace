...
type captureTask struct {
	...
	// checkRules 是会话启动时注入的项目检查规则（不可变；gt-mcp 解析 projects.rules
	// 下发）。空 = check 引擎全程 no-op。
	checkRules []checkrule.CheckRule
	// onAlert 处理一次命中：落库（会话内回看数据源）+ 尽力向探针下发桌面富告警
	// （pipelineService 注入；nil = 单测直接构造 captureTask，无落库/下发通道）。
	onAlert func(alertID string, b checkrule.AlertBundle)
	// check 在 run() 内构造（同 sem），仅主循环 goroutine 调用 inspect。
	check *checkEngine
...
//  3. check.stop()             — 检查规则告警 worker 停并排空（在 finalize 之前）
//  4. dispatcher.Close()       — 在 finalize 之前关闭
//  5. closeCaptureSources      — 在 dispatcher 之前关闭
//  6. tick.Stop()              — 最先执行
func (t *captureTask) run() {
	...
	processDecoded := func(evs []*event.Event) {
		for _, ev := range evs {
			...
			// 项目检查规则：在富化后的事件上求值（可见 name/annotate 注入的 _meta.*），
			// 命中则异步下发告警。inspect 同时维护每方向滑动窗口，故每条事件都要调用；
			// t.check 为 nil（无规则/无下发通道）时跳过。
			if t.check != nil {
				t.check.inspect(ev)
			}
	...
	// 检查规则引擎：项目级规则在解码事件上求值，命中则异步落库并向源会话探针下发告警。
	// 有规则即武装（不再要求存在探针——无探针的 pcap/文件会话同样要能回看命中记录）；
	// 仅当调用方未注入 sink（单测直接构造 captureTask）时跳过。defer stop 在 close(t.done)
	// 之后注册→先于其执行，保证 worker 在 finalize 前停止，且不卡住 Stop(ctx) 的超时预算。
	if active := checkrule.Active(t.checkRules); len(active) > 0 && t.onAlert != nil {
		t.check = newCheckEngine(t.logger, t.sessionID, active, t.onAlert)
		t.check.start()
		defer t.check.stop()
		t.logger.Info("check rules armed", "session_id", t.sessionID, "rules", len(active))
	}
