...
func (s *pipelineService) StartSession(ctx context.Context, req capturecontrol.StartSessionRequest) (capturecontrol.StartSessionResult, error) {
	...
	// onAlert 是检查规则命中后的下发闭包：先把告警落库（平台端 Web 回看「哪些数据
	// 触发了通知」的数据源，对所有会话都记），再尽力向会话所属探针下发桌面富通知。
	// 落库恒执行；探针下发仅在 probeMgr 存在且能解析到绑定探针时进行。全程 best-effort，
	// 任何失败只记日志——运行在 checkEngine worker goroutine，绝不影响抓包主循环。
	onAlert := func(alertID string, b checkrule.AlertBundle) {
		pctx, pcancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer pcancel()
		if err := st.AppendAlert(pctx, sessionID, alertRowFromBundle(b)); err != nil {
			s.logger.Warn("check rule alert: persist failed",
				"session_id", sessionID, "rule_id", b.RuleID, "error", err)
		}
		if s.probeMgr == nil {
			return
		}
		probeID, ok := s.probeMgr.ProbeForSession(sessionID)
		if !ok {
			s.logger.Debug("check rule alert: no probe bound to session, skip desktop notify",
				"session_id", sessionID, "rule_id", b.RuleID)
			return
		}
		detail, err := json.Marshal(b)
		if err != nil {
			s.logger.Warn("check rule alert: marshal bundle failed",
				"session_id", sessionID, "rule_id", b.RuleID, "error", err)
			return
		}
		sendCtx, sendCancel := context.WithTimeout(context.Background(), 12*time.Second)
		defer sendCancel()
		if err := s.probeMgr.SendAlert(sendCtx, probeID, b.Title, b.Message, alertID, detail); err != nil {
			s.logger.Warn("check rule alert: send to probe failed",
				"session_id", sessionID, "probe_id", probeID, "rule_id", b.RuleID, "error", err)
		}
	}
