...
description: "分析 GameTrace 抓取会话的协议血缘（请求-响应配对、父子里层级、消息命名与语义标签）：先查会话绑定的解码插件启用了哪些 semantic_rules，再依据启用的规则选择可用的血缘查询（correlation_id / causation_id / parent_id / meta.msg_name / meta.semantic），按用户需求给出对话式分析结论。当用户想分析抓包事件之间的因果关系、请求响应配对、父子事件层级、追踪某条消息的来龙去脉，或问『这个响应是哪个请求触发的』时调用。"
---
...
对已抓取的会话做**协议血缘分析**：搞清楚事件之间"谁引发谁、谁从属于谁、谁和谁是一组"。血缘字段全部由平台按解码插件声明的 semantic_rules 生成，因此分析的第一步永远是**确认插件启用了什么规则**——有什么规则，才有什么血缘；没启用的规则，对应字段一律为空，不可臆测。

## 何时使用

- 用户问"这个响应/事件是由哪条消息触发的"
- 用户要梳理某次交互的完整请求-响应链路
- 用户要看某条大消息（如快照/批量同步）拆出了哪些子事件
...
semantic_rules 的 4 种 effect 各自点亮一种血缘能力。**插件启用了该规则，就默认规则有效、字段可信**（规则语义正确性由插件开发流程保证，分析时不复查）：
...
| `extract` | 父子里层级 | `parent_id` | 子事件 `parent_id` = 源（父）事件 id，如一条快照消息拆出的多个实体状态 |
...
- 规则 `id`
- `effect.type`（pair / extract / annotate / name）
- `effect` 关键参数：pair 看 `sides`（Side 0 = 请求方、Side 1 = 响应方）与各侧 `key`；extract 看 `children`；name 看 `key`；annotate 看 `tag`
...
- 用户要的"关系"具体是哪种：请求-响应（pair）？父子包含（extract）？还是只是同类消息归类（name/annotate）？
...
| 某事件的全部 extract 子事件 | `parent_id == "<父事件id>"` |
| 某子事件的源消息 | 取该子事件行的 `parent_id`，再 `id == "<该值>"` 查父事件 |
| 按消息名过滤 | `meta.msg_name == "UpgradeReq"` |
| 按语义标签过滤 | `"login" in meta.semantic` |
| 血缘 + 业务字段组合 | `parent_id == "p1" && data.name == "hero"` |

查询要点：

- 事件行本身就带 `correlation_id` / `causation_id` / `parent_id`，先无过滤（或按业务条件）拉一批事件，从行内字段出发做二次过滤往往比盲猜 filter 更快
...
2. **证据链**：按时间序列出参与事件（消息名 + 方向 + 关键业务字段 + 血缘关系），血缘关系显式标注（"↑ 由 UpgradeReq(id=…) 触发" / "↳ Snapshot(id=…) 拆出的子事件"）
