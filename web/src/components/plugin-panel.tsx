...
export function PluginPanel() {
  ...
  // 可选解码插件来自注册表：平台不再扫描任何插件目录，插件实例必须自己注册上来才可见。
  const pluginNames = useMemo(() => plugins.map((p) => p.name), [plugins]);

  useEffect(() => {
    if (!decodeSession && sessions.length > 0) setDecodeSession(sessions[0]!.session_id);
  }, [sessions, decodeSession]);
  useEffect(() => {
    if (!decodePlugin && pluginNames.length > 0) setDecodePlugin(pluginNames[0]!);
  }, [pluginNames, decodePlugin]);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* 离线解码工具栏：默认随原始包视图常开（与后端 raw 工具注册开关对齐）。 */}
      {RAW_DEBUG_ENABLED && (
