...
describe("excludeTokens", () => {
  it("逗号（含中文逗号）与空白都是分隔符", () => {
    expect(excludeTokens("Login,GetBag")).toEqual(["login", "getbag"]);
    expect(excludeTokens("Login，GetBag Notify")).toEqual(["login", "getbag", "notify"]);
    expect(excludeTokens("  a , , b  ")).toEqual(["a", "b"]);
  });
  it("空串得到空数组（= 不剔除）", () => {
    expect(excludeTokens("")).toEqual([]);
    expect(excludeTokens("   ")).toEqual([]);
  });
});

describe("eventExcludesQuery", () => {
  it("命中消息名即剔除，大小写不敏感", () => {
    expect(eventExcludesQuery(event, "playermove")).toBe(true);
    expect(eventExcludesQuery(event, "PLAYERMOVE")).toBe(true);
  });
  it("与搜索看同一份字段包：payload 值、方向、id 都能剔", () => {
    expect(eventExcludesQuery(event, "1001")).toBe(true);
    expect(eventExcludesQuery(event, "client_to_server")).toBe(true);
    expect(eventExcludesQuery(event, "evt-123")).toBe(true);
  });
  it("多个值是「或」：任一词命中即剔除", () => {
    expect(eventExcludesQuery(event, "attack,playermove")).toBe(true);
    expect(eventExcludesQuery(event, "attack move")).toBe(true);
  });
  it("一个词都不命中则保留", () => {
    expect(eventExcludesQuery(event, "attack logout")).toBe(false);
  });
  it("空剔除恒为不剔除", () => {
    expect(eventExcludesQuery(event, "")).toBe(false);
  });
  it("剔除与搜索对同一个词的判定一致", () => {
    for (const term of ["playermove", "1001", "attack"]) {
      expect(eventExcludesQuery(event, term)).toBe(eventMatchesQuery(event, term));
    }
  });
});
