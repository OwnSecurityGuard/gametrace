 ...
 *  - 剔除（黑名单）与搜索看同一个字段包，但多值以逗号或空白分隔、按 OR 合并：
 *    命中任一词即剔除该条。
 *
...
/**
 * 剔除串 → 小写关键词数组。比 fuzzyTokens 多认逗号（含中文逗号）作分隔：
 * 剔除是黑名单，多个值之间天然是「或」，用空白再切一层只会让人不敢用逗号。
 */
export function excludeTokens(exclude: string): string[] {
  return exclude
    .toLowerCase()
    .split(/[,，\s]+/)
    .filter(Boolean);
...
/**
 * 事件的「字段包」：协议名、消息名、方向、id / correlation / causation，
 * 以及 data / meta / analysis 全量序列化文本，小写拼接。
 * 搜索与剔除共用它 —— 两者必须看同一份文本，否则会出现「搜得到、剔不掉」。
 */
export function eventHaystack(ev: DecodedEvent): string {
...
export function eventMatchesQuery(ev: DecodedEvent, query: string): boolean {
  ...
  if (tokens.length === 0) return true;
  return haystackIncludes(eventHaystack(ev), tokens);
}

/**
 * 事件是否要被剔除：字段包命中任一关键词即剔（多值「或」，与搜索的 AND 相反）。
 * 空剔除串恒为 false（不剔），调用方据此把它当透明条件。
 */
export function eventExcludesQuery(ev: DecodedEvent, exclude: string): boolean {
  const tokens = excludeTokens(exclude);
