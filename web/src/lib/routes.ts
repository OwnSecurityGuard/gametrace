...
/** 会话空间的视图（二级 tab）。 */
export type SessionView = "overview" | "connections" | "events" | "states" | "alerts" | "raw";
...
export const SESSION_VIEWS: readonly SessionViewMeta[] = [
  ...
  { id: "alerts", label: "命中提醒", hint: "检查规则命中与触发数据" },
...
/** 「原始包」是解码调试视图；构建期设 VITE_ENABLE_RAW_DEBUG=0 收回暴露面时整个视图不存在。 */
