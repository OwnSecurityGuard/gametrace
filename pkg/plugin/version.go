...
// 插件通过 manifest 的 api_version 字段声明兼容版本（gt.decoder/v2）。
//
...
// 完整格式：gt.decoder/v<数字>，如 gt.decoder/v2。
const apiVersionPrefix = "gt.decoder/"

// ParseManifestAPIVersion 从 manifest 的 api_version 字段提取版本号部分。
// "gt.decoder/v2" → "v2"
// "gt.decoder/v2.3" → "v2.3"
// 非法格式（无前缀或无版本号）返回空串。
...
// 规则：major 版本必须匹配 ProtocolVersion；api_version 格式必须是 gt.decoder/v<数字>。
...
func CheckManifestVersion(m *Manifest) error {
	...
	if declared == "" {
		return fmt.Errorf("manifest api_version %q must match gt.decoder/v<digit>", m.APIVersion)
	}
