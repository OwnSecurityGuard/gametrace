...
func (s *Server) ProbeStartCapture(ctx context.Context, req *pb.ProbeStartCaptureRequest) (*pb.ProbeStartCaptureResponse, error) {
	...
	checkRules, dropped := checkrule.DecodeList(req.GetCheckRulesJson())
	if dropped > 0 {
		slog.Warn("ProbeStartCapture: skipped malformed check rules", "probe_id", req.GetProbeId(), "dropped", dropped)
	}
