...
type CaptureEngine interface {
	...
	// 产出 violations + quality + verdict，并把 validated 证明回写控制库
	// （plugin_validations 表）。
...
type StartSessionRequest struct {
	...
	PluginOwners []string
	// CheckRules 是项目级检查规则（gt-mcp 从 projects.rules 解析后随抓包启动注入）。
	// pipeline 在每条解码事件上求值，命中则向源会话所属探针下发货告警。空 = 不求值。
...
func (s *Server) StartCapture(ctx context.Context, req *pb.StartCaptureRequest) (*pb.StartCaptureResponse, error) {
	...
	engineReq := StartSessionRequest{
		...
		// 既有遗漏修复：StartLeaseCapture / SetSessionPlugin 均映射了 plugin_owners，
		// 唯独 StartCapture 漏了，导致项目成员共用项目插件在普通抓包路径失效。一并补上。
		PluginOwners: req.GetPluginOwners(),
	}
	if rules, dropped := checkrule.DecodeList(req.GetCheckRulesJson()); len(rules) > 0 || dropped > 0 {
		engineReq.CheckRules = rules
		if dropped > 0 {
			slog.Warn("StartCapture: skipped malformed check rules", "session_id", req.GetSessionId(), "dropped", dropped)
		}
	}
