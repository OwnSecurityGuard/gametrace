...
-- 检查规则命中告警：规则在某会话命中一次即落一行，使平台端（Web）能回看
-- 「哪些数据触发了通知」——触发记录 + 各方向触发前最近 N 条快照。
CREATE TABLE IF NOT EXISTS session_alerts (
	...
	indexes := []string{
		...
		// 告警按会话时间倒序分页
		"CREATE INDEX IF NOT EXISTS idx_session_alerts_session_time ON session_alerts(session_id, timestamp)",
	}
