...
// AlertWriter 落地检查规则命中告警（一次命中一行），使平台端 Web 能回看命中记录。
type AlertWriter interface {
	AppendAlert(ctx context.Context, sessionID string, row AlertRow) error
}

// AlertReader 分页查询某会话的命中告警（时间倒序）。
type AlertReader interface {
	QueryAlerts(ctx context.Context, q AlertQuery) ([]AlertRow, int, error)
}
