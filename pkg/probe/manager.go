...
// SendAlert 让探针弹一条桌面通知，并携带一个结构化告警详情包（检查规则命中）。
//
// 与 SendNotify 同为一次性动作（不写 desired、探针离线即失败、不补发）。detailJSON
// 是 checkrule.AlertBundle 的 JSON；alertID 非空时探针会把它存入本地并通过 loopback
// HTTP 服务一个告警详情页、自动打开浏览器展示。detailJSON 为空 / alertID 为空时退化
// 为普通文字通知（旧探针忽略新字段，仍只弹 title/message）。
func (m *Manager) SendAlert(ctx context.Context, probeID, title, message, alertID string, detailJSON []byte) error {
	if _, err := m.store.GetProbe(ctx, probeID); err != nil {
		return fmt.Errorf("probe %s: %w", probeID, err)
	}
	return m.sendAndWait(probeID, &proto.Command{
		Id: m.nextCmdID(),
		Payload: &proto.Command_Notify{Notify: &proto.Notify{
			Title:      title,
			Message:    message,
			AlertId:    alertID,
