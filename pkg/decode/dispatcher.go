	...
	for i, r := range results {
		...
		if len(r.AnalysisMsgpack) > 0 {
			if v, err := event.UnmarshalValueMsgpack(r.AnalysisMsgpack); err == nil {
				...
				// 事件本体（payload/meta）仍然有效，不能因 analysis 解不开整条丢弃；
				// 但「状态投影静默消失」必须像 payload 失败一样能被用户看到。
				d.errs.Add(ErrKindPlugin, req.PacketId, req.Src, req.Dst, "unmarshal msgpack analysis: "+err.Error())
