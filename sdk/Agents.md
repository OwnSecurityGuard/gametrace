...
- `annotate` — `semantic` ∈ request / response / notification / error. Error and notification
  are Event attributes, not Relations. **request/response need no annotate rule**: when the
  event carries no role label (request/response/notification), the host derives one from the
  event direction (client_to_server → request, server_to_client → response; unknown direction
  stays unlabeled; an `error` annotate is a status attribute, not a role, so the default role
  is still appended next to it). Annotate therefore exists to **override** the default — mark
  server pushes as notification, tag failure replies as error — not to obtain the baseline
  roles.
