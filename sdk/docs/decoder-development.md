...
analysis := event.ValueFromMap(map[string]any{
    "_state_changes": []any{
        map[string]any{
            ...
            "op":           "set",
            "path":         "hp",
            "after":        100, // `before` is optional: the platform baseline is
                                 // authoritative; a reported one is only a first-seen fallback
        },
    },
})
...
- `op` (required): one of `set`, `delete`, `merge`. `op` is the **only** thing that decides
  whether a path is removed — `set` to a null value is rejected by the host.
- `path` (required): a logical field identifier of the entity, e.g. `hp`, `inventory.sword`.
  The host stores baselines as exact-match keys per path; it is **not** a GJSON/JSONPath
  expression — wildcards and array indices are not interpreted. Dots are a naming
  convention owned by the plugin.
- `after`: required by `op` — `set`: any non-null value; `merge`: must be an object, which
  the host shallow-merges onto the baseline (the projection stores the merged full value,
  not your fragment); `delete`: not needed and ignored even if sent.
- `before` (optional): **not authoritative**. The platform baseline holds the real
  "before" value and enriches it; a plugin-supplied `before` is only a fallback used when
  the entity/path is seen for the first time.
...
The host validates each item; an item missing a required field, with an unknown `op`, or
violating the `op`/`after` contract is skipped with a warning and does not abort the write.
Do not fabricate `_state_changes` for events that do not actually mutate state — empty
state is fine.
