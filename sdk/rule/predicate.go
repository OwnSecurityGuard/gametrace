...
type Predicate struct {
	...
	// 单条件
	Path  string `yaml:"path,omitempty" json:"path,omitempty"`   // GJSON path，如 data.targetId
	Op    Op     `yaml:"op,omitempty" json:"op,omitempty"`
	Value any    `yaml:"value,omitempty" json:"value,omitempty"` // 字面量：标量或数组
}

// UnmarshalJSON 与 UnmarshalYAML 对称，支持两种 when 写法：
//
//	[ {path: seqId, op: neq, value: 0}, ... ]   // 数组 → 隐式 all
//	{ "all": [...] } / { "any": [...] } / 叶子对象
//
// 项目检查规则经 encoding/json 持久化与传输（control.sqlite 的 rules 列、
// 抓包启动的 check_rules_json），故需要 JSON 形态的简写支持。
...
// Validate 报告该谓词节点的声明合法性（导出形态，供宿主在保存项目检查规则时校验）。
// Path 为相对定位（不含规则与 when 前缀）。
func (p *Predicate) Validate() []Issue { return p.validate() }
