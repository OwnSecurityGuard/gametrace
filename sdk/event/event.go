...
//
// op 与 after 的契约（与 contract.yaml、宿主基线语义一致）：
//   - set    ：after 必填、任意非空 Value（移除 path 用 delete，不支持 set 成 null）；
//   - merge  ：after 必填且必须是 Object，宿主将其与基线已有值浅合并；
//   - delete ：after 不需要（即使携带也被忽略）。
//
// before 不是权威值：平台基线持有并富化真实的「变更前」，插件上报的 before 仅作首见兜底。
// path 是该实体的逻辑字段标识（基线按精确匹配取键），不是 GJSON/JSONPath 表达式，
// 不支持通配符与数组下标；嵌套只是插件自己的命名约定。
...
// Validate 校验 StateChange 的最小必填字段、op 取值及 op/after 对应关系。
...
func (sc StateChange) Validate() error {
	...
	switch sc.Op {
	case "set":
		if sc.After.Kind == Null {
			return fmt.Errorf("set requires a non-null after (use op=delete to remove %q)", sc.Path)
		}
	case "merge":
		if sc.After.Kind != Object {
			return fmt.Errorf("merge requires after to be an object")
		}
	case "delete":
		// after 不需要。
	default:
...
type Event struct {
	...
	// Trace 描述事件的执行链路上下文（Debug Trace）
	Trace TraceContext
