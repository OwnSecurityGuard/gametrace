...
// parse as an lp frame (e.g. a foreign protocol on the port) is dropped
...
func (d *decoder) decode(req *pb.DecodeRequest, stream pb.Decoder_DecodeV2Server) error {
	...
	for {
		...
		if !ok {
			// Incomplete frame (or the header/length rejected the flow):
			// wait for more segments on a later call.
			break
		}
		if err := d.emit(stream, inputID, flowID, f); err != nil {
			return err
		}
		st.Consume(n)
	}
